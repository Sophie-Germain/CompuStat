...
## applications:
## 1) Markov variance reduction via importance sampling
...
## Libraries
  ...
  ui <- dashboardPage(
      ...
      dashboardSidebar(
          ...
          column(11, offset = 1,
                 helpText(h3("Nota: "),
                          p(
                          paste0(" CM Variance correction with IS and CI.",
                                 " Bias correction with Bootstrap.",
  ...
  server <- function(input, output) {
      ...
      ## Estimar betha1/betha2 usando Bootstrap
      bootstrap_test <- function(d1, d2){
          ...
          fit2    <- lm(y~x, data = d2)
          ## Output statistic
          fit1$coefficients[2]/fit1$coefficients[1]
      }
      ...
      output$boots_plot <- renderPlot({
          ...
          non_skwe <- data.frame(theta = 2*skwe[,1] - mean(na.omit(skwe[,1])))
